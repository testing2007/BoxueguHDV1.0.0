...
class BXGCourseOutlineVC: UIViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if let videoModel = BXGVODPlayerListManager.shared.currentVideo {
            viewModel.updateVideoStatus(videoModel: videoModel, status: .changed)
        }
    ...
    let viewModel: BXGCourseOutlineViewModel
    ...
    init(courseModel fromCourseModel: BXGStudyCourseModel, phaseModel fromPhaseModel: BXGCoursePhaseModel, moduleModel fromModuleModel: BXGCourseModuleModel) {
        viewModel = BXGCourseOutlineViewModel(courseModel: fromCourseModel, phaseModel: fromPhaseModel, moduleModel: fromModuleModel)
    ...
    func loadData() {
        if let courseId = viewModel.courseModel.id, let moduleId = viewModel.moduleModel.id {
    ...
    func isSectionExam(indexPath: IndexPath) -> Bool {
        guard let moduleModel = self.viewModel.outlineModuleModel,
...
//        self.navigationController?.pushViewController(vc, animated: true)
        ...
        let vc = BXGDownloadSelectVC(moduleModel: self.viewModel.outlineModuleModel!, selectVCDelegate:self)
        self.navigationController?.pushViewController(vc, animated: true)
...
//        }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        var title:String?
        if let sectionItems = self.viewModel.outlineModuleModel?.sectionItems {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return (self.viewModel.outlineModuleModel?.sectionItems?.count) ?? 0
    }
...
extension BXGCourseOutlineVC : BXGDownloadSelectVCDelegate {
    ...
    func onConfirmDownload(model: BXGCourseOutlineModuleModel?) {
        ...
        if let model=model {
            var courseType:Int
                switch self.viewModel.courseModel.type {
            ...
            BXGDownloadManager.instance.addDownloadItem(courseOutlineModuleModel: model,
                                                        courseName: self.viewModel.courseModel.courseName ?? "" ,
                                                        courseType: courseType,
                                                        courseImageUrl: self.viewModel.courseModel.smallImgPath ?? "",
                                                        phaseName: self.viewModel.phaseModel.phaseName ?? "");
...
extension BXGCourseOutlineVC: BXGPlayerListManagerDelegate {
    ...
    func videoReadyToPlay(manager: BXGVODPlayerListManager, video: BXGVODPlayerVideoModel, index: Int) {
        ...
        viewModel.updateVideoStatus(videoModel: video, status: .begin)
    }
    ...
    func videoPlayDone(manager: BXGVODPlayerListManager, video: BXGVODPlayerVideoModel, index: Int) {
        viewModel.updateVideoStatus(videoModel: video, status: .finished)
    }
    
    func videoDidChange(manager: BXGVODPlayerListManager, video: BXGVODPlayerVideoModel, index: Int) {
        viewModel.updateVideoStatus(videoModel: video, status: .changed)
    }
